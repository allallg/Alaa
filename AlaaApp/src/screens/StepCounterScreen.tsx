  ...
  return (
    <View style={[styles.container, { backgroundColor: isDarkMode ? '#1a1a1a' : '#f8f9fa' }]}>
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        ...
        >
          <Text style={[styles.stepCounterLabel, { color: isDarkMode ? '#bdc3c7' : '#7f8c8d' }]}>
            צעדים היום
          </Text>
          <Text style={[styles.stepCount, { color: isDarkMode ? '#fff' : '#2c3e50' }]}>
            {steps.toLocaleString()}
          </Text>
          <Text style={[styles.goalText, { color: isDarkMode ? '#95a5a6' : '#95a5a6' }]}>
          ...
          <View style={[styles.progressContainer, { backgroundColor: isDarkMode ? '#34495e' : '#ecf0f1' }]}>
            <Animated.View
              style={[
                ...
                {
                  width: `${progress * 100}%`,
                  backgroundColor: progress >= 1 ? '#27ae60' : '#3498db',
                },
              ]}
            />
          </View>
          
          <Text style={[styles.progressText, { color: isDarkMode ? '#bdc3c7' : '#7f8c8d' }]}>
            {Math.round(progress * 100)}% מהמטרה
          </Text>
          
          {progress >= 1 && (
            <Text style={styles.congratsText}>
              🎉 כל הכבוד! השגת את המטרה!
            </Text>
          )}
        </Animated.View>
        ...
        <View style={styles.statsContainer}>
          {stats.map((stat, index) => (
            ...
            >
              ...
              <Text style={[styles.statValue, { color: isDarkMode ? '#fff' : '#2c3e50' }]}>
                {stat.value}
              </Text>
        ...
        <View style={[styles.statusContainer, { backgroundColor: isDarkMode ? '#2c3e50' : '#fff' }]}>
          <Text style={[styles.statusIcon, { color: isWalking ? '#27ae60' : '#95a5a6' }]}>
            {isWalking ? '🚶‍♂️' : '🧍‍♂️'}
          </Text>
          <Text style={[styles.statusText, { color: isDarkMode ? '#fff' : '#2c3e50' }]}>
            {isWalking ? 'הולך עכשיו...' : 'לא פעיל'}
          </Text>
        </View>
        ...
        <View style={styles.buttonContainer}>
          <TouchableOpacity
            ...
            style={[
              styles.actionButton,
              { backgroundColor: isWalking ? '#e74c3c' : '#27ae60' },
            ]}
          >
            <Text style={styles.actionButtonText}>
              {isWalking ? '⏸️ הפסק' : '▶️ התחל ללכת'}
            </Text>
          </TouchableOpacity>
        ...
        <View style={[styles.tipsContainer, { backgroundColor: isDarkMode ? '#34495e' : '#fff' }]}>
          <Text style={[styles.tipsTitle, { color: isDarkMode ? '#fff' : '#2c3e50' }]}>
            💡 טיפים לפעילות
          </Text>
          <Text style={[styles.tipsText, { color: isDarkMode ? '#bdc3c7' : '#7f8c8d' }]}>
            • מומלץ ללכת לפחות 10,000 צעדים ביום
          </Text>
          <Text style={[styles.tipsText, { color: isDarkMode ? '#bdc3c7' : '#7f8c8d' }]}>
            • קח הפסקות קצרות כל שעה
          </Text>
          <Text style={[styles.tipsText, { color: isDarkMode ? '#bdc3c7' : '#7f8c8d' }]}>
            • השתמש במדרגות במקום מעלית
          </Text>
        </View>
      </ScrollView>
    </View>
